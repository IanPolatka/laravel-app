            ...
            <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <select name="home_team_id" id="home_team_id" class="form-control" onChange="window.location.href=this.value">
                            ...
                            @foreach($teams as $team)
                              ...
                              <option value="/tennis-boys/{{$showcurrentyear[0]}}/{{ $team->school_name }}">{{ $team->school_name }}</option>
                    ...
                    <div class="form-group">
                        ...
                        <select name="home_team_id"class="form-control" onChange="window.location.href=this.value">
                            ...
                            @foreach($years as $year)
                              ...
                              <option value="/tennis-boys/{{ $year->year }}" @if ($showcurrentyear[0] == $year->year) selected @endif>
                                {{ $year->year }}
                              </option>
                ...
                @if (Auth::user())
                    ...
                    <div class="col-lg-2">
                        ...
                        <p><a class="pull-right btn btn-success" href="/tennis-boys/create">Create Match</a></p>
                    ...
                    <ul class="list-group">
                        @foreach($tennis as $item)
                            ...
                            <li class="list-group-item">
                                {{ $item->date }} |
                                <a href="/tennis-boys/{{ $showcurrentyear[0] }}/{{ $item->away_team->school_name }}">
                                    
                                    {{ $item->away_team->school_name }}</a> 
                                    
                                <a href="/tennis-boys/{{ $showcurrentyear[0] }}/{{ $item->home_team->school_name }}">{{ $item->home_team->school_name }}</a>
                                @if ($item->tournament_title)
                                    <small>({{ $item->tournament_title }})</small>
                                @endif
                                <span class="pull-right"><a href="/tennis-boys/{{ $item->id }}/edit">Edit</a></span>&nbsp;&nbsp;&nbsp;
                            </li>
