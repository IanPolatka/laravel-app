...
@forelse ($soccer as $item)
    ...
    <li><a href="/soccer-boys/match/{{ $item->id }}">

        <div class="date">
            {{ Carbon\Carbon::parse($item->date)->format('l') }} {{ Carbon\Carbon::parse($item->date)->format('M j, o') }}
        </div>
                                @if ($selectedteam[0]['id'] == $item['away_team_id'])
                                    @ 
                                    <div class="team-logo">
                                ...
                                @else
                                    ...
                                    <span class="pull-right">
                                        ...
                                        @if ($selectedteamid[0] == $item->losing_team)
                                            <strong><span style="color: red">L</span></strong>
                                        @elseif ($selectedteamid[0] == $item->winning_team)
                                        ...
                                        @if ($item->away_team_final_score > $item->home_team_final_score)
                                            {{ $item->away_team_final_score }} - {{ $item->home_team_final_score }}
                                        @else
                                            {{ $item->home_team_final_score }} - {{ $item->away_team_final_score }}
                                        @endif
